...
/////////////////////////////////////////////////////////////
	...
	switch (that.config.type) 
	{
		...
		case "GarageDoorOpener": 
		{
			...
			if ( (currentDoorState.config.interface_name != "Z-Wave") 
				&& ((that.config.openingValue === undefined) || (that.config.closingValue === undefined) || (that.config.stoppedValue === undefined)))
				{
					globals.log(red("*Warning* - openingValue, closingValue, or stoppedValue not defined in config.json for GarageDoorOpener with reference: " + that.config.ref));
			...
			//  globals.log(red("lockValue is: " + targetDoorState.config.closedValue))
			// globals.log(red("unlockValue is: " + targetDoorState.config.openValue))
			// globals.log(red("config is: " + JSON.stringify(that.config)))
				
				
			currentDoorState	
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
					{
						if(HomeKitObject.config.interface_name == "Z-Wave") // Values of Z-Wave Barrier class.
						{
							switch(newHSValue)
							{
								case(newHSValue == 255)	:	
										{	HomeKitObject.updateValue(0);	break;	} // Open
								
								case(newHSValue == 0)	:	
										{	HomeKitObject.updateValue(1);	break;	} // Closed
								
								case(newHSValue == 254)	:	
										{	HomeKitObject.updateValue(2);	break;	} // Opening
								
								case(newHSValue == 252)	:	
										{	HomeKitObject.updateValue(3);	break;	} // Closing
								
								case(newHSValue == 253 || ( (1 <= newHSValue) && (newHSValue <= 99)) )	:	
															{	HomeKitObject.updateValue(4);	break;	} // Stopped
															
								default:
								{
									throw new RangeError(red("Error in GarageDoorOpener - value out of range! Z-Wave Value is: " + newHSValue));
								}
							}
						}
						else
						{
							// globals.log(red("GarageDoorOpener Received Value from HomeSeer: " + newHSValue));
							...
							switch(parseFloat(newHSValue))
							{
								...
								case(HomeKitObject.config.closedValue)	:	
										{	HomeKitObject.updateValue(1);	break;	} // Closed
								case(HomeKitObject.config.openingValue)	:	
										{	HomeKitObject.updateValue(2);	break;	} // Opening
								case(HomeKitObject.config.closingValue)	:	
										{	HomeKitObject.updateValue(3);	break;	} // Closing
								case(HomeKitObject.config.stoppedValue)	:	
										{	HomeKitObject.updateValue(4);	break;	} // Stopped
								default:
								{
									globals.log(red("*Debug* - configuration value for garage door opener are: " + JSON.stringify(HomeKitObject.config) ));
			...
			targetDoorState
				...
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
					{
						// globals.log(red("Updating Target Value, received: " + newHSValue));
						switch(parseFloat(newHSValue))
						{
							case(HomeKitObject.config.openValue)	:	
									{	HomeKitObject.updateValue(0);	break;	} // Open
		...
		case "Lock": {
			...
			// Use a user-defined value first, then try to determine the value from HomeSeer's 'Control Use' settings.
				...
				switch(true)
				{
					...
					case (thisOff !== null):  // Else use the HomeSeer On value
					{
						thisService.onControl.offValue = thisOn
						break;
					}							
					default:  // Else just try 255 which is the Z-Wave Default!
					{
