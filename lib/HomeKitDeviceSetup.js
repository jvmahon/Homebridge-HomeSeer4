...
globals.sendHS = function(ref, value) 
{
	...
	if( ref == null || value == null) throw new SyntaxError("Called globals.sendHS with incorrect parameters. Need to specify both a reference and a value. Reference was: " + ref + ", Value was: " + value);
	...
	switch (that.config.type) 
	{
		...
		case "BinaryFan":
		{
			...
			thisService
				...
				// This should actually be done by a switch statement testing for null just in case a valid value is zero. For valid values of zero, the or statemetn would move onto the next choice event though it shouldn't do that!				
				onControl.onValue = onControl.findCommandValue("OnAlternate") || onControl.findCommandValue("On") || onControl.config.onValue;
				onControl.offValue = onControl.findCommandValue("Off")
				if (onControl.offValue === null) onControl.offValue = onControl.config.offValue;
				
				try {
					assert((onControl.offValue === 0), "Error - Off value for Switch/Outlet is not 0")
				} catch (error) { "A non-zero Off Value for a Switch is Unusual!" };
					...
					.on('set', function(newValue, callback)
								{
									...
									switch(newValue == true) // Compare it to true because you might also get a 0 or 1 due to Active characteristic!
									{
										case(true): // turn on.  Active Characteristic for Fan = 1, which evaluates to truthy
										{
											...
											globals.setHSValue(thisService.onControl.HSRef, thisService.onControl.onValue)
										...
										case(false): // turn off. Active Characteristic for Fan = 0, which evaluates to falsy
										{
											...
											globals.setHSValue(thisService.onControl.HSRef, thisService.onControl.offValue) 
											break;
										}
									}
									callback(null);
