	...
	switch (that.config.type) 
	{
		...
		case "Thermostat": {
			...
			thermostatService.getCharacteristic(Characteristic.TargetHeatingCoolingState)
				.on('set', (level, callback) =>
					{
						that.transmitToHS(level, that.config.controlRef);
						thermostatService.mode = level;
						...
						var heatingTemp = (that.config.heatingSetpointRef === undefined) ? 0 : getHSValue(thermostatService.config.coolingSetpointRef);
						var coolingTemp = (that.config.coolingSetpointRef === undefined) ? 0 : getHSValue(thermostatService.config.heatingSetpointRef);
						if (thermostatService.config.temperatureUnit == "F")
							{ 
						...
						switch(level)
						{
							...
							case(1): //mode heating
								{
									thermostatService.getCharacteristic(Characteristic.TargetTemperature)
										.setValue(heatingTemp); 
									break
								}
							case(2): // mode cooling
								{
									thermostatService.getCharacteristic(Characteristic.TargetTemperature)
										.setValue(coolingTemp); 
									break;
								}
							case(3): // mode auto
								{
									/*
									that.log(chalk.cyan.bold("*Debugging data:" +
										"\nCooling Min: " + thermostatService.getCharacteristic(Characteristic.CoolingThresholdTemperature).props.minValue +
										"\nCooling Max: " + thermostatService.getCharacteristic(Characteristic.CoolingThresholdTemperature).props.maxValue +
										"\nHeating Min: " + thermostatService.getCharacteristic(Characteristic.HeatingThresholdTemperature).props.minValue +
										"\nHeating Max: " + thermostatService.getCharacteristic(Characteristic.HeatingThresholdTemperature).props.maxValue +
										"\nTarget Min: " + thermostatService.getCharacteristic(Characteristic.TargetTemperature).props.minValue +
										"\nTarget Max: " + thermostatService.getCharacteristic(Characteristic.TargetTemperature).props.maxValue
									));
									*/
			...
			thermostatService.getCharacteristic(Characteristic.TemperatureDisplayUnits)
				.on('change', ()=>
					{
						console.log(chalk.magenta.bold("*Warning * -  Changing Hardware Display Units Does Nothing!"));
					})
