...
Service.prototype.updateUsingHSReference = function(value) 
{
	...
	switch (that.config.type) 
	{
		...
		case "Fan": {
			...
			var fanService = new Service.Fanv2;
			fanService.isPrimaryService = true;
			fanService.displayName = "Service.Fanv2"
			
			fanService.getCharacteristic(Characteristic.Active)
				.updateUsingHSReference(that.config.ref)
				.setConfigValues(that.config)
				.on('set', that.setHSValue.bind(fanService.getCharacteristic(Characteristic.Active)));


			if (globals.findControlPairByCommand(that.config.ref, "Dim"))
			{
            ...
            //globals.log("   fanService: " + JSON.stringify(fanService));
		...
		case "Lightbulb": 
		{
			...
			if (globals.findControlPairByCommand(that.config.ref, "Dim"))
			{
				globals.log(magenta("*Debug * - Bulb Can Dim, ref: " + that.config.ref ));
            ...
            //globals.log("   lightBulbService: " + JSON.stringify(lightbulbService));
