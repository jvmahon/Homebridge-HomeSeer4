...
/* Characteristic.prototype.setSupportedLevels = function(HomeKitObject) 
{
	...
	try
	{
		this.isBinary = ((HSutilities.findControlPairByCommand(this.config.ref, "Dim") === undefined) && (HSutilities.findControlPairByCommand(this.config.ref, "DimFan") === undefined) )? true: false;
	...
	switch (that.config.type) 
	{
		...
		case "Lock": {
			...
			if (	that.config.lockedStatusValues.includes(that.config.unlockValue) 
					...
					{
						throw new SyntaxError(red("*ERROR* - Error setting up lock. The unlockedStatusValues includes your locking value, or else the lockedStatusValues inlcudes your unlocking value. Please report this on github if you can't resolve it!"));
					};
			...
			if (	(that.config.lockedStatusValues.includes(that.config.lockValue) === false)
					...
					{
						throw new SyntaxError(red("*ERROR* - Error setting up lock. The unlockedStatusValues is missing your unlocking value, or else the lockedStatusValues is missing ylocking value. Please report this on github if you can't resolve it!"));
					}
