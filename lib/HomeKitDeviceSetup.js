...
/////////////////////////////////////////////////////////////
	...
	switch (that.config.type) 
	{
		...
		case "GarageDoorOpener": 
		{
			...
			thisService.control.lockValue = thisService.config.closedValue || thisService.control.findCommandValue("DoorLock");
			thisService.control.unlockValue = thisService.config.openValue || thisService.control.findCommandValue("DoorUnLock");
			
			thisService.displayName = "Service.GarageDoorOpener";
			thisService.getCharacteristic(Characteristic.CurrentDoorState)
				...
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
					{
						if(HomeKitObject.config.interface_name == "Z-Wave") // Values of Z-Wave Barrier class.
						{
							switch(newHSValue)
							{
								...
								default:
								{
									throw new RangeError(red("Error in GarageDoorOpener - value out of range!"));
								}
							}
						}
						else
						{
							...
							switch(newHSValue)
							{
								case(thisService.control.unlockValue)	:	{	HomeKitObject.updateValue(0);	break;	} // Open
								case(thisService.control.lockValue)	:		{	HomeKitObject.updateValue(1);	break;	} // Closed
								...
								default:
								{
									throw new RangeError(red("Error in GarageDoorOpener type - HSvalueChanged value out of range!"));
								}
							}
						}
			...
			thisService.getCharacteristic(Characteristic.TargetDoorState)
				.updateUsingHSReference(that.config.ref)
				.setConfigValues(that.config)
				.on('set', function(value, callback, context)
					{
						switch(value)
						{
							case 0: {this.sendHS(thisService.control.unlockValue); break;} // Door Open
							case 1: {this.sendHS(thisService.control.lockValue); break; } // Door Closed
						}
