...
/////////////////////////////////////////////////////////////
	...
	//////////////////			
			...
			var lockControl = 	lockService.getCharacteristic(Characteristic.LockTargetState)	
									.setConfigValues(that.config)

						var thisLockValue = lockControl.findCommandValue("DoorLock");
						switch(true)
						{
							case (lockControl.config.lockValue !== undefined): // If a user has specified a value, that overrides all else!
							{
								lockControl.lockValue = (lockControl.config.lockValue)
								break;
							}
							case (thisLockValue !== null):  // Else use the HomeSeer On-Alternate value
							{
								lockControl.lockValue = thisLockValue
								break;
							}
							default:  // Else just try 255 which is the Z-Wave Default!
							{
								globals.log(red("*Warning* - Unable to find an 'lockValue' value for the device: " + lockControl.config.ref + " defaulting to 255"));
								lockControl.lockValue =   255;
							}
						}

						var thisUnlockValue = lockControl.findCommandValue("DoorUnLock")
						switch(true)
						{
							case (lockControl.config.unlockValue !== undefined): // If a user has specified a value, that overrides all else!
							{
								lockControl.unlockValue = (lockControl.config.unlockValue)
								break;
							}
							case (thisUnlockValue !== null):  // Else use the HomeSeer Off value
							{
								lockControl.unlockValue = thisUnlockValue;
								break;
							}							
							default:  // Else just try 255 which is the Z-Wave Default!
							{
								globals.log(red("*Warning* - Unable to find an 'unlockValue' value for the device: " + lockControl.config.ref + " defaulting to 0"));
								lockControl.unlockValue =   0;
							}
						}				
				debuglog("Determined lock value: '" + lockControl.lockValue + "', and unlock value: '" + lockControl.unlockValue +",");
	///////////////////			
				lockService
				.setConfigValues(that.config)
				.updateUsingHSReference(that.config.ref)
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
				{
					var lockedvalues = [65, 129, 255, lockControl.lockValue];
					var unlockedvalues = [0, 1, 16, 17, 32, 33, 64, 128, lockControl.unlockValue];
			...
			lockService.getCharacteristic(Characteristic.LockTargetState)
				...
				.on('set', function(value, callback, context)
							{
								(value == true) ? this.sendHS(lockControl.lockValue) : this.sendHS(lockControl.unlockValue)
								callback(null);
							} );
