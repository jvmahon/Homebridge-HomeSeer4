	...
	switch (that.config.type) 
	{
		...
		case "BinaryLight":
		{
			...
			thisService
				...
				onControl.onValue = onControl.findCommandValue("OnAlternate") || onControl.findCommandValue("On");
				onControl.offValue = onControl.findCommandValue("Off");
				
				assert((onControl.offValue == 0), "Error - Off value for Switch/Outlet is not 0")
			...
			onControl
				...
				.on('set', function(newHSValue, callback, context)
							{
								(newHSValue) ? onControl.sendHS(onControl.onValue) : onControl.sendHS(onControl.offValue)
								callback(null);
							} );
			services.push(thisService);
		...
		case "Lock": {
			...
			case "DimmingLight":
			{
				...
				thisService.isDimmer = (globals.findControlPairByCommand(that.config.ref, "Dim") != null) ? true: false;
				...
				onControl.onValue = onControl.findCommandValue("OnAlternate") || onControl.findCommandValue("On");
				onControl.offValue = onControl.findCommandValue("Off");
				globals.log(yellow("* Debug * - LightBulb On value is: " + onControl.onValue + " and Off value is: " + onControl.offValue));
				
				onControl
					.on('set', function(value, callback)
								{

									if (brightControl == undefined)
									{
										//globals.log(cyan("Debug - Received a new On Value of: " + value + " when On was set to: " + onControl.value + "  while HomeSeer array has a value of :" + globals.getHSValue(onControl.HSRef)));
									}
									else
									{
										//globals.log(cyan("Debug - Received a new On Value of: " + value + " when On was set to: " + onControl.value + " and while Brightness was " + brightControl.value + " and while HomeSeer array has a value of :" + globals.getHSValue(onControl.HSRef)));
									}
									switch(value)
									{
										case(true): // turn on
										{
												...
												if (onControl.value) {
													// globals.log("Debug - Already On, so do nothing!");
														break; // send nothing if already on
															}
												
												if (onControl.config.interface_name == "Z-Wave") 
														{ 
															// globals.log("Debug - Sending ZWave On value of 255!");
															onControl.sendHS(255) ; 
															globals.setHSValue(onControl.HSRef, 255) // assume success and before a HomeSeer response, force the array to have the new value.

														}
													else
														{ 
															// globals.log("Debug - Sending Non-ZWave On value of: " + onControl.config.onValue);
															globals.sendHS(onControl.config.onValue )
															globals.setHSValue(onControl.HSRef, onControl.config.onValue)
														}
														break;
										}
										case(false): // turn off
										{
											...
											onControl.sendHS(0);
											globals.forceHSValue(onControl.HSRef, 0) 
											break;
										}
									}
						...
						.on('set', function(value, callback, context)
								{
									...
									globals.log(chalk.magenta("Received a new Brightness Value of: " + value + " when On was set to: " + onControl.value + " and while Brightness was " + brightControl.value));
					...
					thisService.on('HSvalueChanged', function(newValue, HomeKitObject)
						{
							if (brightControl == undefined)
							{
							// globals.log(chalk.blue("Updating HSValueChanged for a Binary light with name: " + HomeKitObject.name + " to a new value: " + newValue + " when On was set to: " + onControl.value));
							}
							else
							{
							// globals.log(chalk.blue("Updating HSValueChanged for a Dimming light with name: " + HomeKitObject.name + " to a new value: " + newValue + " when On was set to: " + onControl.value + " and while Brightness was " + brightControl.value));
							}
							switch(true)
							{
		...
		case "Fan": 
		{
						...
						.on('set', function(value, callback, context)
								{
									switch(value)
									{
										case(true): // turn on
										{
											...
											if (activeControl.config.interface_name == "Z-Wave") 
													{ 
														// globals.log("Debug - Sending ZWave On value of 255!");
														activeControl.sendHS(255) ; 
														globals.setHSValue(activeControl.HSRef, 255) // assume success and before a HomeSeer response, force the array to have the new value.

													}
												else
													{ 
														// globals.log("Debug - Sending Non-ZWave On value of: " + activeControl.config.onValue);
														globals.sendHS(activeControl.config.onValue )
														globals.setHSValue(activeControl.HSRef, activeControl.config.onValue)
													}
											break;
										}
										case(false): // turn off
										{
											// globals.log("Debug - Turning Off!");
											activeControl.sendHS(0);
											globals.forceHSValue(activeControl.HSRef, 0) 
											break;
										}
									}
			...
			// Now is the tricky part - handle updates coming from HomeSeer!
				...
				thisService.on('HSvalueChanged', function(newValue, HomeKitObject)
					{
						// globals.log(chalk.blue("Updating a Adjustable Fan"));
						switch(true)
						{
							case(newValue == 0):// Zero is universal for turning off, so just turn light off
							{
								HomeKitObject.getCharacteristic(Characteristic.Active).updateValue(false); 
								break;
							}
							case(newValue == 255): // 255 is the Z-Wave value for "last value" - just turn on
							{
								HomeKitObject.getCharacteristic(Characteristic.Active).updateValue(true); 
								break;
							}
							default: // any other value, figure if it is just to adjust RotationSpeed, or to turn on!
							{
								...
								{
									case(true): // already on, so just adjust RotationSpeed
									{
										HomeKitObject.getCharacteristic(Characteristic.RotationSpeed).updateValue(newValue); 
										break;
									}
									case(false): // was off, so turn on and adjust RotationSpeed.
									{
										HomeKitObject.getCharacteristic(Characteristic.Active).updateValue(true); 
										HomeKitObject.getCharacteristic(Characteristic.RotationSpeed).updateValue(newValue); 
										break;
