...
var debuglog = function(string) { globals.log(chalk.yellow("*Debug* " + string))};
// var debug = null;
var errorlog = function (string) { globals.log(chalk.red("*Error* " + string))}
//var errorlog = null;
var infolog = function (string) { globals.log(chalk.blue("*Info:* " + string))}
...
/////////////////////////////////////////////////////////////
	...
	switch (that.config.type) 
	{
				...
				.on('set', function(value, callback, context)
					{
						...
						{
							case 0: 
							{ 	// 0 = HomeKit Valve Closed
								valveService.getCharacteristic(Characteristic.Active).sendHS( that.config.closeValve); 
								break; 
							}
							case 1: 
							{ 	// 1 = HomeKit Valve Open
								valveService.getCharacteristic(Characteristic.Active).sendHS( that.config.openValve) ; 
								break; 
							} 
						}
				...
				.on('set', function(value, callback, context)
					{
						switch (value)
						{
							case 0: 
							{ 	// 0 = HomeKit Valve Closed
								valveService.getCharacteristic(Characteristic.InUse).sendHS( that.config.closeValve); 
								break; 
							}
							case 1: 
							{ 	 // 1 = HomeKit Valve Open
								valveService.getCharacteristic(Characteristic.InUse).sendHS( that.config.openValve) ; 
								break; 
							}
						}
						callback(null);
				...
				.on('set', function(value, callback, context)
					{
						switch(value)
						{
							case 0: 
							{ 	// Door Open
								garageDoorOpenerService.getCharacteristic(Characteristic.TargetDoorState).sendHS(that.config.openDoorValue); 
								break;
							}
							case 1: 
							{ 	// Door Closed
								garageDoorOpenerService.getCharacteristic(Characteristic.TargetDoorState).sendHS(that.config.closeDoorValue); 
								break; 
							}
						}
			...
			thisService
				...
				// Select from a hierarchy of possible 'on' values. Usually this is retrieved from HomeSeer control Pairs using findCommandValue.				
						...
						switch(true)
						{
							...
							case (thisAlternate !== null):  // Else use the HomeSeer On-Alternate value
							{
								onControl.onValue = thisAlternate
								break;
							}
							case (thisOn !== null):  // Else use the HomeSeer On value
							{
						...
						switch(true)
						{
							...
							case (thisOff !== null):  // Else use the HomeSeer Off value
							{
		...
		case "SecuritySystem":
		{
			...
			securitySystemService
				...
				switch (true)
					...
					case ( HomeKitObject.config.alarmValues.includes(parseFloat(newHSValue)) ): 
						{ 	// 4 = HomeKit Triggered. No "TargetState" to be set!
							HomeKitObject.getCharacteristic(Characteristic.SecuritySystemCurrentState).updateValue(4); // 4 = HomeKit Alarm Triggered
							break; 
						} 
					default:
						{ 
							errorlog("Value not processed when handling change to securitySystemService" );
							break; 
						} 
			...
			var SecuritySystemTargetState = securitySystemService.getCharacteristic(Characteristic.SecuritySystemTargetState);
			...
			SecuritySystemTargetState
				...
				.on('set', function(newHSValue, callback, context)
				{
						switch (newHSValue)
						{
						case 0: { SecuritySystemTargetState.sendHS( that.config.armStayValue); break; }// 0 = HomeKit Stay Arm
						case 1: { SecuritySystemTargetState.sendHS( that.config.armAwayValue); break; } // 1 = HomeKit Away Arm
						case 2: { SecuritySystemTargetState.sendHS( that.config.armNightValue); break; } // 2 = HomeKit Night Arm
						case 3: { SecuritySystemTargetState.sendHS( that.config.disarmValue); break; } // 3 = HomeKit Disarmed
	...
	//////////////////			
				// Select from a hierarchy of possible 'Lock' and 'unLock" values. Usually this is retrieved from HomeSeer control Pairs using findCommandValue.				
			var targetControl = 	lockService.getCharacteristic(Characteristic.LockTargetState)	
									.setConfigValues(that.config)

						var thisLockValue = targetControl.findCommandValue("DoorLock");
						switch(true)
						{
							case (targetControl.config.lockValue !== undefined): // If a user has specified a value, that overrides all else!
							{
								targetControl.lockValue = (targetControl.config.lockValue)
								break;
							}
							case (thisLockValue !== null):  // Else use the HomeSeer On-Alternate value
							{
								targetControl.lockValue = thisLockValue
								break;
							}
							default:  // Else just try 255 which is the Z-Wave Default!
							{
								errorlog("*Warning* - Unable to find an 'lockValue' value for the device: " + targetControl.config.ref + " defaulting to 255");
								targetControl.lockValue =   255;
							}
						}

						var thisUnlockValue = targetControl.findCommandValue("DoorUnLock")
						switch(true)
						{
							case (targetControl.config.unlockValue !== undefined): // If a user has specified a value, that overrides all else!
							{
								targetControl.unlockValue = (targetControl.config.unlockValue)
								break;
							}
							case (thisUnlockValue !== null):  // Else use the HomeSeer Off value
							{
								targetControl.unlockValue = thisUnlockValue;
								break;
							}							
							default:  // Else just try 255 which is the Z-Wave Default!
							{
								errorlog("*Error* - Unable to find an 'unlockValue' value for the device: " + targetControl.config.ref + " defaulting to 0");
								targetControl.unlockValue =   0;
							}
						}				
				debuglog("Determined lock value: '" + targetControl.lockValue + "', and unlock value: '" + targetControl.unlockValue +",");
	///////////////////			
				lockService
				.setConfigValues(that.config)
				.updateUsingHSReference(that.config.ref)
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
				{
					 // Most of the following values are from Z-Wave Application Command Class Specification, SS13781, Version 13
					 // But several came from users!
					var lockedvalues = [65, 129, 255, targetControl.lockValue];
					var unlockedvalues = [0, 1, 16, 17, 32, 33, 64, 128, targetControl.unlockValue];
					
					switch(true)
					{
						case(lockedvalues.includes(parseFloat(newHSValue)) ): // Zwave Locked
						{
							debuglog("Setting to Locked");
						...
						default:
						{
							debuglog("Setting Lock to Jammed");
				...
				.on('set', function(value, callback, context)
							{
								switch(parseFloat(value))
								{
									case 0:
									{	// Unlocked / Unsecured
										targetControl.sendHS(targetControl.unlockValue)
										break;
									}
									case 1:
									{ 	// Locked / Secured
										targetControl.sendHS(targetControl.lockValue)
										break;
									}

									default:
									{
									errorlog("Invalid value when trying to 'set' LockTargetState. Value is: " + value);
									throw new SyntaxError("Invalid value when setting LockTargetState, value is: " + value);
									}
								}
								callback(null);
							} );
