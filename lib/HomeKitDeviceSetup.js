				...
				.on('HSvalueChanged', function(newHSValue, HomeKitObject)
					{
						if(HomeKitObject.config.interface_name == "Z-Wave") // Values of Z-Wave Barrier class.
						{
							switch(parseFloat(newHSValue))
							{
								...
								case(newHSValue == 254)	:	
										{	currentDoorState.updateValue(2);	break;	} // Opening
								
								case(newHSValue == 252)	:	
										{	currentDoorState.updateValue(3);	break;	} // Closing
								
								case(newHSValue == 253 || ( (1 <= newHSValue) && (newHSValue <= 99)) )	:	
															{	currentDoorState.updateValue(4);	break;	} // Stopped
						...
						else
						{
							...
							switch(parseFloat(newHSValue))
							{
								case(currentDoorState.config.openValue)	:	
										{	
											targetDoorState.updateValue(0);	
											currentDoorState.updateValue(0);	
											break;	
										} // Open
								case(currentDoorState.config.closedValue)	:	
										{	
											targetDoorState.updateValue(1);
											currentDoorState.updateValue(1);	
											break;	
										} // Closed
								case(currentDoorState.config.openingValue)	:	
										{	currentDoorState.updateValue(2);	break;	} // Opening
								case(currentDoorState.config.closingValue)	:	
										{	currentDoorState.updateValue(3);	break;	} // Closing
								case(currentDoorState.config.stoppedValue)	:	
										{	currentDoorState.updateValue(4);	break;	} // Stopped
								default:
								{
									globals.log(red("*Debug* - configuration value for garage door opener are: " + JSON.stringify(currentDoorState.config) ));
