	...
	switch (that.config.type) 
	{
				...
				.on('set', function(value, callback, context)
					{
						...
						switch (value)
						{
						case 0: { HomeSeerData.sendDataValue( that.config.closeValve, that.config.ref); break; }// 0 = HomeKit Valve Closed
						case 1: { HomeSeerData.sendDataValue( that.config.openValve, that.config.ref) ; break; } // 1 = HomeKit Valve Open
						}
				...
				.on('set', function(value, callback, context)
					{
						switch(value)
						{
							case 0:  // Command to HomeSeer to Open Door
								{
									HomeSeerData.sendDataValue(targetDoorState.config.openValue, that.config.ref); 
									break;
								}
							case 1:  // Command to HomeSeer to Close Door
								{
									HomeSeerData.sendDataValue(targetDoorState.config.closedValue, that.config.ref); 
									break; 
								}
						}
					...
					.on('set', function(value, callback, context)
					{
						// globals.log(yellow("*Debug* - TargetPosition value is : " + value));
							switch( value)
							{
								...
								{
									HomeSeerData.sendDataValue(that.config.closedValue, that.config.ref)
									callback(null);
									break;
								}
								case 1:
								{
									HomeSeerData.sendDataValue(that.config.openValue, that.config.ref)
									callback(null);
									break;
								}
								default:
		...
		case "VariableWindow": 
		{
			...
			thisService.getCharacteristic(Characteristic.TargetPosition)
					.on('set', function(value, callback, context)
					{
						HomeSeerData.sendDataValue(value, that.config.ref)
						callback(null);
					} );		
				...
				.on('set', function(newHSValue, callback, context)
							{
								switch(newHSValue == true)
								{
									case (true):
									{
										HomeSeerData.sendDataValue(onControl.onValue, onControl.config.ref)
										break;
									}
									case (false):
									{
										oHomeSeerData.sendDataValue(onControl.offValue, onControl.config.ref)
										break;
									}
									default:
									{
		...
		case "SecuritySystem":
		{
			...
			securitySystemService.getCharacteristic(Characteristic.SecuritySystemTargetState)
				...
				.on('set', function(newHSValue, callback, context)
				{
						switch (newHSValue)
						{
						case 0: { HomeSeerData.sendDataValue( that.config.armStayValue, that.config.ref); break; }// 0 = HomeKit Stay Arm
						case 1: { HomeSeerData.sendDataValue( that.config.armAwayValue, that.config.ref); break; } // 1 = HomeKit Away Arm
						case 2: { HomeSeerData.sendDataValue( that.config.armNightValue, that.config.ref); break; } // 2 = HomeKit Night Arm
						case 3: { HomeSeerData.sendDataValue( that.config.disarmValue, that.config.ref); break; } // 3 = HomeKit Disarmed
		...
		// globals.log(yellow("*Debug* - Lock Configuration Values are: " + JSON.stringify(that.config )));
				...
				.on('set', function(value, callback, context)
							{
								...
								(value) ? HomeSeerData.sendDataValue(this.config.lockValue, that.config.ref ) : HomeSeerData.sendDataValue(this.config.unlockValue, that.config.ref)
						...
						.on('set', function(value, callback, context)
								{
									...
									if((HomeSeerData.getValue(thisService.multilevelControl.HSRef) != 255) && (HomeSeerData.getValue(thisService.multilevelControl.HSRef) != value))
									{
			...
			case "Speaker": // As of iOS 13, not supported in the Home App so this is pretty much useless!
			{
				...
				volumeControl
					.on('set', function(value, callback, context)
							{
								HomeSeerData.sendDataValue(value, volumeControl.config.ref)
								callback(null); //must always finish with the callback(null);
							} );
