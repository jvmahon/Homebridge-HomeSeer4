			...
			// Use a user-defined value first, then try to determine the value from HomeSeer's 'Control Use' settings.
				...
				{	
					...
					thisService.colorTemperatureControl.on('HSvalueChanged', function(newValue, HomeKitObject)
					{
						...
						var updatedValue = Math.min(Math.max(newValue, thisService.colorTemperatureControl.props.minValue), thisService.colorTemperatureControl.props.maxValue )		

						thisService.colorTemperatureControl.updateValue(updatedValue);
					})
				}
