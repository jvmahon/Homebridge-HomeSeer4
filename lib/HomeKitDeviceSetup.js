	...
	switch (that.config.type) 
	{
		...
		case "BinaryFan":
		{
			...
			thisService
				...
				onControl.offValue = onControl.findCommandValue("Off") || onControl.config.offValue;
			...
			onControl
				...
				.on('set', function(newHSValue, callback, context)
							{
								(newHSValue == true) ? onControl.sendHS(onControl.onValue) : onControl.sendHS(onControl.offValue)
								callback(null);
							} );
			services.push(thisService);
