...
Service.prototype.updateUsingHSReference = function(value) 
{
	...
	switch (that.config.type) 
	{
		...
		case "Switch": 
		{
            ...
            //globals.log("   switchService: " + JSON.stringify(switchService))
		...
		case "Outlet": {
            ...
            //globals.log("   outletService: " + JSON.stringify(outletService))
		...
		*/
				...
				.on('set', function(value, callback, context)
							{
								...
								globals.log(yellow("Interface: " + this.config.interface_name));
								callback(null);
							} );
		...
		case "Fan": {
			...
			thisService.getCharacteristic(Characteristic.Active)
				.updateUsingHSReference(that.config.ref)
				.setConfigValues(that.config)
				.on('set', that.setHSValue.bind(thisService.getCharacteristic(Characteristic.Active)));
			...
			if (HSutilities.findControlPairByCommand(that.config.ref, "Dim"))
			{
				thisService.addCharacteristic(new Characteristic.RotationSpeed())
					.setConfigValues(that.config)
					.updateUsingHSReference(that.config.ref)
					.on('set', that.setHSValue.bind(thisService.getCharacteristic(Characteristic.RotationSpeed)))
					.setProps({maxValue:that.config.levels});		
			}
            //globals.log("   thisService: " + JSON.stringify(thisService));
			services.push(thisService);
				...
				.on('set', function(value, callback, context)
							{
								if(value == true) // turn On
								{
									if (this.config.interface_name == "Z-Wave")
										{
											if(globals.getHSValue(this.HSRef) == 0)  // only transmit if it was Off!
											{
												this.sendToHomeSeer(255) 
												// Set the HSValue array to 255 because if this is a dimmable, the code is about to process the dimming function and it needs to know whether the device just got a 255!
												globals.forceHSValue(this.HSRef, 255)
											}
										}
										else {  
											if(globals.getHSValue(this.HSRef) == 0)  this.sendToHomeSeer(this.config.onValue)
										}
								}
								else { this.sendToHomeSeer(0) }  // Turn Off. 0 Seems to be universal for Off!
								callback(null);
							} );

			if (HSutilities.findControlPairByCommand(this.HSRef, "Dim"))
			{
