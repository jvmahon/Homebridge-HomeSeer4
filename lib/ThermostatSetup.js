...
exports.setupThermostat = function (that, services)
{
		...
		thermostatService
			.on('HSvalueChanged', function(newHSValue, HomeKitObject)
			{
				...
				console.log(chalk.blue("Thermostat SetPoint changed"));
					...
					let HSheatingTemp = globals.getHSValue(homekitObject.config.heatingSetpointRef).value;
					let HScoolingTemp = globals.getHSValue(homekitObject.config.coolingSetpointRef).value;
	...
	thermostatService.getCharacteristic(Characteristic.TargetHeatingCoolingState)
		...
		.on('HSvalueChanged', function(newHSValue, HomeKitObject)
			{
			globals.log(yellow("* Debug * - Received TargetHeatingCoolingState CHANGE event from HomeSeer with value: " + newHSValue))
