...
exports.setupThermostat = function (that, services)
{
	...
	if (deviceConfig.heatingMode === undefined)
	{
		...
		deviceConfig.heatingMode.Off = globals.findCommandValue(deviceConfig.controlRef, "ThermModeOff")
		deviceConfig.heatingMode.Heat = globals.findCommandValue(deviceConfig.controlRef, "ThermModeHeat")
		deviceConfig.heatingMode.Cool = globals.findCommandValue(deviceConfig.controlRef, "ThermModeCool")
		deviceConfig.heatingMode.Auto = globals.findCommandValue(deviceConfig.controlRef, "ThermModeAuto")
		
		// deviceConfig.heatingMode = { "Off": 0, "Heat": 1, "Cool": 2, "Auto": 3}
		
	}
	// globals.log(yellow("*Debug* - heating mode property names: " + Object.getOwnPropertyNames(deviceConfig.heatingMode) ))
	// globals.log(yellow("*Debug* - heating modes: " + JSON.stringify(deviceConfig.heatingMode) ))
	...
	thermostatService.getCharacteristic(Characteristic.TargetHeatingCoolingState)
		.on('set', function(value, callback, context)
		{
				...
				switch(value)
				{
					case 0: 
						{
							globals.sendHS(deviceConfig.controlRef, parseInt(deviceConfig.heatingMode.Off), true); 
							break;
						}
					case 1: 
						{
							globals.sendHS(deviceConfig.controlRef, parseInt(deviceConfig.heatingMode.Heat), true);
							break;
						}
					case 2: 
						{
							globals.sendHS(deviceConfig.controlRef, parseInt(deviceConfig.heatingMode.Cool), true);
							break;
						}
					case 3: 
						{
							globals.sendHS(deviceConfig.controlRef, parseInt(deviceConfig.heatingMode.Auto), true);
							break;
						}
				}
				...
				var heatingTemp = (deviceConfig.heatingSetpointRef === undefined) ? undefined : globals.getHSValue(thermostatService.config.heatingSetpointRef);
				var coolingTemp = (deviceConfig.coolingSetpointRef === undefined) ? undefined : globals.getHSValue(thermostatService.config.coolingSetpointRef);
				if (thermostatService.temperatureUnit == "F")
					{ 
			...
			var timer;
					...
					switch(true)
					{
						case (data.newValue == 0) && (thermostatService.temperatureUnit == "F"): 
							{	
								globals.log(yellow("Thermostat Hardware Display Unit Changed to Celsius, but HomeSeer is set to Fahrenheit. Resetting Hardware Display Unit to Fahrenheit."));
						...
						case  (data.newValue == 1) && (thermostatService.temperatureUnit == "C"): 
							{	
								globals.log(yellow("Thermostat Hardware Display Unit Changed to Fahrenheit, but HomeSeer is set to Celsius. Resetting Hardware Display Unit to Celsius."));
								timer = setTimeout( function() 
									{	thermostatService.getCharacteristic(Characteristic.TemperatureDisplayUnits).updateValue(1);				
									}, 3000)
								break;	
