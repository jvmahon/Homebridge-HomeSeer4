...
exports.setupThermostat = function (that, services)
{
	...
	thermostatService.thermostatType = 0;
		...
		if ( (deviceConfig.coolingSetpointRef !== undefined) && (deviceConfig.coolingSetpointRef != 0)) 	
			...
			let statusField = globals.allHSDevices.HSdeviceStatusInfo.find( (element) => 
					{ 
						return (element.ref == deviceConfig.ref ) 
					})
					.status;
			...
			switch(true)
			{
				...
				default:
				{
					throw new RangeError(red("Error setting up Thermostat. Scale is not Celsius or Fahrenheit"), "ThermostatSetup.js");
				}
			}
