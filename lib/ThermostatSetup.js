...
exports.identifyThermostatData = function (thermostatRoot, allAccessories)
{
		...
		var RootStatusInfo = [];
		RootStatusInfo = globals.allHSDevices.HSdeviceStatusInfo.find( (element) => {return(element.ref == thermostatRoot)})
		globals.log(yellow("*Debug* - Found Status Info for reference: " + thermostatRoot + " as " + JSON.stringify(RootStatusInfo)));
		...
		var ThermostatConfigurationData = function (base)
				{
					this.type = "Thermostat";
					this.name = RootStatusInfo.name;
					this.uuid_base = base;
					this.ref = null;
					this.stateRef = null;
					this.humidityRef = null;
					this.coolingSetpointRef = null;
					this.heatingSetpointRef = null;
					this.controlRef = null;
					this.temperatureUnit = "F";
					return this;
				}
		var configuration = new ThermostatConfigurationData("ThermostatRef" + thermostatRoot);
		
		for(var currentDeviceReference of associatedDevices)
		{
			var currentDeviceStatusInfo = globals.allHSDevices.HSdeviceStatusInfo.find( (element) => {return(element.ref == currentDeviceReference)})
			...
			switch(currentDeviceStatusInfo.device_type.Device_Type)
			{
				...
				{
					configuration.stateRef = currentDeviceReference; // 1:0
					break;
				}
				case 2: // Temperature
				{
					switch(currentDeviceStatusInfo.device_type.Device_SubType)
					{
						case 1: 
						{
							configuration.ref = currentDeviceReference; // 2:1
							break;
						}
						case 5: 
						{
							if (currentDeviceStatusInfo.name.toLowerCase().includes("indoor")) 	configuration.humidityRef = currentDeviceReference; // 2:5
							break;
						}
					}
					break;
				...
				{
					configuration.controlRef = currentDeviceReference; // 3:1
					break;
				}
				...
				case 6: // Setpoints
				{
					switch(currentDeviceStatusInfo.device_type.Device_SubType)
					{
						...
						{
							configuration.heatingSetpointRef = currentDeviceReference; // 6:1
							break;
						}
						case 2: // Cooling Setpoint
						{
							configuration.coolingSetpointRef = currentDeviceReference; // 6:2
							break;
						}
					}
				}				
				...
				default:
				{
					console.log(green("Executed the Switch statement for info: " + currentDeviceStatusInfo.device_type.Device_Type));
				}
			}

		}

		console.log(chalk.cyan("Discovered Configuration is:  " + JSON.stringify(configuration)));
