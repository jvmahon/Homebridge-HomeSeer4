	...
	switch (that.config.type) 
	{
		...
		case "LightSensor": 
		{
			...
			ambientLight
				...
				.setConfigValues(that.config)	
				.on('HSvalueChanged', (newHSValue, homekitObject) => { 
							updateValue =  Math.min(Math.max(newHSValue, ambientLight.props.minValue), ambientLight.props.maxValue )		
							homekitObject.updateValue(updateValue)
						})				
			break;
		}
		...
		case "TemperatureSensor": 
		{
			...
			// globals.log(chalk.yellow(`Minimum value: ${that.config.minCelsius} celsius, maximum value: ${that.config.maxCelsius} celsius.` ));
