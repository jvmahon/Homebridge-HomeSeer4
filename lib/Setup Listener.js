...
module.exports.setupHomeSeerTelnetPort = function ()
{
	...
	globals.telnetClient.on('connect', () =>
		{
			...
			let authorization = "au," + ( globals.platformConfig["login"] || "default" ) + "," + (globals.platformConfig["password"] || "default") +"\r";
	...
	globals.telnetClient.on('data', (data) => 
		...
		for (const thisUpdate of allPendingUpdates)
		{
			...
			case("error"):
			{
				globals.log(red("Warning - Error Received on Telnet port. If this occurred during startup, this maybe due to bad password / login information in your config.json file. Parsed data is: " + JSON.stringify(myData) ));
				globals.log(red("Unparsed data was: " + data));

				break;
			};
			default:
				{
					globals.log(red("Warning - Unexpected Data Received on Telnet port. Parsed data is: " + JSON.stringify(myData) ));
					globals.log(red("Unparsed data was: " + data));
					break;
				}
			}
		...
		globals.telnetClient.on('error', (data) => 
			...
			// But it is only once every 5 minutes, so it isn't much overhead and its an extra check to make sure data remains consistent.
					...
					const 	statusURL = new URL(globals.platformConfig["host"]);
						...
						{
							promiseHTTP({ uri: statusURL.href, json:true, strictSSL:false})
							.then( function(json) 
								{
							...
							.catch(function(err)
								{
									globals.log(red("HomeSeer poll attempt failed with error: " + err));
