...
module.exports.setupHomeSeerTelnetPort = function ()
{
	...
	networkInformation = url.parse(globals.platformConfig["host"]);
	
	globals.log(cyan("Host for ASCII Interface set to: " + networkInformation.hostname + " at port " + ASCIIPort));
	
	globals.telnetClient = net.createConnection({port:ASCIIPort, host:networkInformation.hostname});
			...
			case("DC"): // Handle received data
				{	
					...
					if( globals.statusObjects[myData.ref])	
					{
						...
						{
							globals.log(chalk.blue("Emitting HSvalueChanged for object: " + thisHomeKitObject.config.name));
							thisHomeKitObject.emit('HSvalueChanged', myData.newValue, thisHomeKitObject);
						}
					} 
					break;
				};
	...
	globals.telnetClient.on('close', () => 
		{
			...
			setTimeout( ()=>
			{
				...
				try
				{
					...
					// globals.telnetClient = net.createConnection({port:ASCIIPort, host:networkInformation.host});
					globals.telnetClient.connect({port:ASCIIPort, host:networkInformation.hostname});
				} catch(err)
				{
