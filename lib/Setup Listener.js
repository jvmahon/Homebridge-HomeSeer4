...
module.exports.setupHomeSeerTelnetPort = function ()
{
	...
	client.on('connect', () =>
		{
			globals.log(cyan("Successfully connected to ASCII Control Interface of HomeSeer. Instant Status Enabled."));
			numAttempts = 1
	...
	client.on('data', (data) => 
		{
			...
			var myData = new HSData(data.toString().slice(0, -2).split(","));
						globals.log(cyan("Received ASCII data, Data is :" + JSON.stringify(myData) ));
