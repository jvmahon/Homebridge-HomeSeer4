...
module.exports.setupHomeSeerTelnetPort = function ()
{
	...
	globals.telnetClient.on('data', (data) => 
		...
		var allPendingUpdates = data.toString().split(/(\r\n|\n|\r)/gm)/ // multiple pending updates can be received at once!
		globals.log(chalk.red("Number of pending updates: " + allPendingUpdates.length));
		
		for (const thisUpdate of allPendingUpdates)
		{
			var myData = new HSData( thisUpdate.split(","));
			...
			case("DC"): // Handle received data
				{	
					...
					if( globals.statusObjects[myData.ref])	
					{
						...
						for (var thisCharacteristic of statusObjectGroup)
						{
							globals.log(chalk.blue("Emitting HSvalueChanged"));
							thisCharacteristic.emit('HSvalueChanged', myData.newValue, thisCharacteristic);
						}
					} 
					break;
				};
