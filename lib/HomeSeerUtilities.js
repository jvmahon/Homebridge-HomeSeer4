	...
	try  { 
		...
		config.accessories.forEach((it) => {
			...
			if (it.name === undefined) {
				it.name = HomeSeerData.getLocation(findRef) + " " + HomeSeerData.getName(findRef)
				if( it.name.length > 64)  { 
						globals.log(red("Warning - Excess name length, trimming default name to 64 characters for device: " + it.name));
						it.name = it.name.slice(0,64); 
			...
			if (it.batteryRef === 0) {
				...
				var deviceBattery = HomeSeerData.findBattery(it.ref);
			}
			if (deviceBattery)
			{
