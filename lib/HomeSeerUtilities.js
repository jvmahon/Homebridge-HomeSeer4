...
globals.findControlPairByCommand = function (ref, command)
{
	...
	let validCommands = [ 
		...
		try {		
				...
				// globals.log(cyan("This command's controls are: " + JSON.stringify(allThisDevicesControls[thisCommandControl])));
				...
				if (thisCommandControl == -1) { 
							globals.log(yellow("Device: " + ref + " does not support command: " + command));
							return(null);
					};
		}
		catch(err)
