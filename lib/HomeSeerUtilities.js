...
var checkConfig = function(config, allHSDevices)
{
	...
	{ 
		...
		{
			error = red("Too many accessories and events in config.json file. Bridge maximum is 100. You have specified: " + (config.accessories.length + config.events.length));
					...
					if (config.accessories[i].can_dim === undefined)
						{
							...
							if ( CurrentHomeSeerDevice.device_type_string && ((CurrentHomeSeerDevice.device_type_string == "Z-Wave Switch Binary") || (CurrentHomeSeerDevice.device_type_string == "Z-Wave Switch")))
							{
