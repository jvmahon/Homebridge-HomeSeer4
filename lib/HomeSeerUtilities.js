...
checkConfig = function(config)
{
	...
	{ 
		...
		for (var i in config.accessories)
		{
			...
			// console.log(chalk.magenta.bold("Type of thisItem: " + typeof thisItem + ", with property: " + thisItem[property] + " of type " + typeof thisItem[property]));
				...
				switch(thisItem[property])
				{
					...
					case "coolingThresholdRef":
					case "heatingThresholdRef":
					{
		...
		switch(config.accessories[i].type)
		{
			...
			case ("Valve"):
			{
				config.accessories[i].openValue 		= 	(config.accessories[i].openValve === undefined) 	? 255 	: config.accessories[i].openValve;
				config.accessories[i].closedValue 		=  	(config.accessories[i].closeValve === undefined) 	? 0 	: config.accessories[i].closeValve;
			}	
			case ("Valve"):
			{
		...
		//	Make sure the specified HomeSeer device references are all numbers
			...
			if((config.accessories[i].uses99Percent != null) && (typeof(config.accessories[i].uses99Percent) != "boolean"))
			{
				switch(config.accessories[i].uses99Percent.toLowerCase())
				{
					...
					default:
					{
						var error = chalk.red.bold('HomeBridge config.json file error - Incorrect uses99Percent setting. Must be one of "true" or "false" You have setting of: "' 
			...
			if(config.accessories[i].temperatureUnit != null)
			{
				...
				switch (config.accessories[i].temperatureUnit)
				{
					...
					default:
					{
					var error = chalk.red.bold('HomeBridge config.json file error - Incorrect temperatureUnit setting. Must be one of "C" or "F" You have setting of: "' 
								+ config.accessories[i].temperatureUnit + '" for device with name ' + config.accessories[i].name
								+', and HomeSeer reference of: ' + config.accessories[i].ref)
						throw new SyntaxError(error);
					}
				}
			...
			if((config.accessories[i].binarySwitch != null) && (typeof(config.accessories[i].binarySwitch) != "boolean"))
			{
				switch(config.accessories[i].binarySwitch.toLowerCase())
				{
					...
					default:
					{
						var error = chalk.red.bold('HomeBridge config.json file error - Incorrect .binarySwitch setting. Must be one of "true" or "false" You have setting of: "' 
