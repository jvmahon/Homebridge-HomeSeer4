...
globals.findControlPairByCommand = function (ref, command)
{
	...
	let validCommands = [ 
		...
		try {		
				...
				let allThisDevicesControls = globals.allHSDevices.controlPairs.find( (element) => {return(element.ref == ref)})
				// globals.log(yellow("Control pairs are: " + JSON.stringify(allThisDevicesControls)));
				if (allThisDevicesControls === undefined)
					{
						globals.log(red("LIne 74: Device: " + ref + " does not support command: " + command +", returning undefined"));
						return undefined;
					};
									
				//Then check if the ControlPairs has a pair matching the specified command.
				var thisCommandControl = allThisDevicesControls.ControlPairs.find( (element) => {return ( element.ControlUse == command)});
...
var checkConfig = function(config)
{
	...
	{ 
		...
		for (let thisAccessoryConfigData of config.accessories)
		{
			...
			{
				let isDimmer = (globals.findControlPairByCommand(thisAccessoryConfigData.ref, "Dim") === undefined) ? false : true;
				if (isDimmer)
				{
					globals.log(yellow("Changing LightBulb type to DimmingLight for reference: " + thisAccessoryConfigData.ref));
					thisAccessoryConfigData.type = "DimmingLight"
				}
				else
				{
					globals.log(yellow("Changing LightBulb type to BinaryLight for reference: " + thisAccessoryConfigData.ref));
			...
			if (CurrentHomeSeerDevice === undefined) 
								{
			...
			{
				thisAccessoryConfigData.name = CurrentHomeSeerDevice.location.slice(0,32) + " - " + CurrentHomeSeerDevice.name;
...
function findBattery(findRef)
{
	try
	{
		...
		if (thisDevice === undefined ) return (0);
		...
		if (rootDevice === undefined) return (0);
