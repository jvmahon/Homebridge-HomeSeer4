			...
			if (deviceBattery)
			{
				if (it.batteryRef === undefined ||  it.batteryRef == null)  {
					...
					it.batteryRef = deviceBattery;
						...
						globals.log(`Wrong battery Specified for device Reference #: ${cyan(it.ref)}. You specified reference: ${cyan(it.batteryRef)}, but correct device reference appears to be: ${cyan(deviceBattery)}. Fixing error.`);
