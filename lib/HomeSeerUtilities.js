...
var checkConfig = function(config, allHSDevices)
{
	...
	try 
	{ 
		console.log(green("Checking Configuration Data"));
		...
		for (var i in config.accessories)
		{
			...
			let deviceIndex = allHSDevices.HSdeviceStatusInfo.findIndex( (element, index, array)=> {return (element.ref == findRef)} );
			if (deviceIndex != -1) 
				{ 
					var CurrentHomeSeerDevice = allHSDevices.HSdeviceStatusInfo[deviceIndex] 
				}
				else
				{
			...
			if (deviceBattery)
			{
				...
				{
					console.log(yellow("Device Reference #: ") + cyan(config.accessories[i].ref)
				...
				{
					console.log(yellow("You specified battery reference: "+ config.accessories[i].batteryRef + " for device Reference #: " + config.accessories[i].ref 
			...
			switch(config.accessories[i].type)
			{
				...
				case ("Fan"):
				{
					...
					if (config.accessories[i].can_dim === undefined)
						...
						// console.log(magenta("*Debug* for: " + config.accessories[i].name + ", can_dim = " + config.accessories[i].can_dim));
			...
			initialData
			console.log("Config.json property information updated from: " + cyan(initialData) + " to: " + cyan( finalData));	
		}	
...
function findBattery(findRef)
{
	try
	{
		...
		// console.log(red("Global is: " + Object.getOwnPropertyNames(globals.allHSDevices.HSdeviceStatusInfo) ))
