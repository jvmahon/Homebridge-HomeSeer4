		...
		for (let thisAccessoryConfigData of config.accessories)
		{
			...
			switch(thisAccessoryConfigData.type)
			{
				...
				case ("Fan"):
				{
					...
					globals.log(magenta(JSON.stringify(globals.findControlPairByCommand(thisAccessoryConfigData.ref, "Dim"))));
					
					if (thisAccessoryConfigData.can_dim === undefined)
						{
							thisAccessoryConfigData.can_dim = true; // default to true unless you know its a Z-Wave Binary Switch
							...
							{
								thisAccessoryConfigData.can_dim = false;
							}
						}
						else
						{
							thisAccessoryConfigData.can_dim = parseBoolean(thisAccessoryConfigData.can_dim)
						}
						// this.log(magenta("*Debug* for: " + thisAccessoryConfigData.name + ", can_dim = " + thisAccessoryConfigData.can_dim));
					// If its a dimmable type, then also check if it only uses the 1-99 % range (i.e., is it a Z-Wave device)
					if (thisAccessoryConfigData.can_dim == true) 
					{
						
						if ( CurrentHomeSeerDevice.device_type_string.toLowerCase().indexOf("multilevel") == (-1) )
						{
							globals.log(
									magenta( "* Warning * - Check can_dim setting for device named: ") + cyan(thisAccessoryConfigData.name) 
									+ magenta(", and HomeSeer reference: ") + cyan(thisAccessoryConfigData.ref) 
