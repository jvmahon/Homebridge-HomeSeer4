...
exports.sendToHomeSeer = function (level, that )
{
		var url;
		var error = null;
		...
		/*
		globals.log(yellow("\n\nPlatform configuration is: " + Object.getOwnPropertyNames(globals.platformConfig) 
						+	"\nThat structure is: " + Object.getOwnPropertyNames(that)
						+	"\nGlobals configuration is: " + Object.getOwnPropertyNames(globals)
						+	"\nGlobals.api configuration is: " + Object.getOwnPropertyNames(globals.api)));	
		*/							
		
		// Uncomment for Debugging
		// globals.log ("** Debug ** - Called sendToHomeSeer with level %s for UUID %s", level, that.displayName);
		...
		if (!that.UUID) {
			...
			// Uncomment any UUID's actually used!
				switch( that.UUID)
				{
					...
					{
						if (that.config.closeValue) // If its a valve
							{
								switch (level)
								{
							...
							} else // it is a fan.
							{
								switch (level)
								{
								case 0: { transmitValue = that.config.offValue; break; }// 0 = HomeKit Valve Closed
								case 1: { transmitValue = that.config.onValue; break; } // 1 = HomeKit Valve Open
								}
							}
					
						// globals.log(cyan("* Debug * Valve open / close values are: " +that.config.openValve + ", " + that.config.closedValue));
						break;
					}						
					...
					case(Characteristic.Brightness.UUID ): 
					{
						...
						if(globals.getHSValue(that.HSRef) == 255)
						{
							performUpdate = false;
						}
						...
						*/
						...
						globals.forceHSValue(that.HSRef, transmitValue); 

						// that.updateValue(transmitValue); // Assume success. This gets corrected on next poll if assumption is wrong.
						// globals.log ("          ** Debug ** called for Brightness update with level %s then set to transmitValue %s", level, transmitValue); 
					...
					case(Characteristic.TargetPosition.UUID):
					{
						...
						// globals.log("Set TransmitValue for WindowCovering %s to %s ", that.displayName, transmitValue);
						break;
					}	
					...
					case(Characteristic.TargetDoorState.UUID):
					{
						...
						globals.log("Set TransmitValue for lock characteristic %s to %s ", cyan(that.displayName), cyan(transmitValue));
						break;
					}
					case (Service.Thermostat.UUID):
					{
						globals.log(cyan("* Service.Thermostat Update Currently does nothing!"));
					...
					{
						// globals.log(magenta("**Debug** - case HeatingThresholdTemperature, configuration: " + that.config));
					...
					{
						// globals.log(magenta("**Debug** - case CoolingThresholdTemperature, configuration: " + that.config));
						...
						// globals.log(magenta("**Debug** - Dummy Target Temperature Function Level is: " +level + " transmitted as: " + transmitValue + " temperatureUnit is: " + that.config.temperatureUnit));
...
/* 
					case(Characteristic.TargetTemperature.UUID):
					{
						...
						globals.log(magenta("**Debug** - Fix so that Target Temp is mode dependent"));
...
*/
						...
						if (level == false) 
							{
								...
								globals.forceHSValue(that.HSRef, that.config.offValue); // assume success and set to offValue. Will get corrected by the HomeSeer response if this is the wrong assumption!
						}
						else // turn on!
						{
							if(globals.getHSValue(that.HSRef) == that.config.offValue)	// if it is currently off, then turn fully on.
							{
								// if it is off, turn on to full level.
								transmitValue = that.config.onValue;
								globals.forceHSValue(that.HSRef, 255); // Set to '255 even if you use a different onValue and let the system correct itself by a status update from HomeSeer
							}
							else // If it appears to be on, then send same value!
							{
								...
								transmitValue = globals.getHSValue(that.HSRef); // if it is already on, then just transmit its current value
					...
					default:
					{
						globals.log (red("*** PROGRAMMING ERROR **** - Service or Characteristic UUID not handled by setHSValue routine for: "
						+ that.displayName
						));
		...
		if (isNaN(transmitValue)) 
		 ...
		 url = globals.platformConfig["host"] + "/JSON?request=controldevicebyvalue&ref=" + transmitRef + "&value=" + transmitValue;
 
		 // For debugging
		 //globals.log ("Debug - Called setHSValue has URL = %s", url);
 
		 // globals.log("Sending URL %s", url);

		if (performUpdate)
		 {
			 globals.log("Transmitting to HomeSeer device: " + cyan(transmitRef) +", a new value: " + cyan(transmitValue));

			 promiseHTTP(url)
				.then( function(htmlString) {
						// globals.log(that.displayName + ': HomeSeer setHSValue function succeeded!');
						// callback(null);
...
*/
		...
		switch(true)
		{
			...
			case(characteristicObject.UUID == Characteristic.CurrentAmbientLightLevel.UUID):
                //globals.log("DEBUG - Case Characteristic.CurrentAmbientLightLevel");
			case(characteristicObject.UUID == Characteristic.CurrentRelativeHumidity.UUID):
                //globals.log("DEBUG - Case Characteristic.CurrentRelativeHumidity");
			case(characteristicObject.UUID == Characteristic.TargetRelativeHumidity.UUID):
                //globals.log("DEBUG - Case Characteristic.TargetRelativeHunmidity");
			case(characteristicObject.UUID == Characteristic.BatteryLevel.UUID):
			{
                //globals.log("DEBUG - Case Characteristic.BatteryLevel");
				newValue = globals.getHSValue(characteristicObject.HSRef);
