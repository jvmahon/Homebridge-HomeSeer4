...
var typeProperties =
{
	...
	"Lock":
	{
		Properties: 
		{
			...
			"lockedStatusValues":[65, 129, 255], // An array of values to indicate door is locked.
			"unlockedStatusValues":[0, 1, 16, 17, 32, 33, 64, 128], // An array of values to indicate door is unlocked.	
			"interface_name":null			
		},
		
		mandatory: ["type", "name", "ref", "uuid_base"],
		
		setDefaults:["lockValue", "unlockValue", "lockedStatusValues", "unlockedStatusValues"]
	},
	...
	"Fan":
	{
		...
		setDefaults:["onValue", "offValue", "levels"]
	},
	...
	"Window":
	{
		...
		setDefaults:["obstructionClearValues", "openValue", "closedValue", "levels"]
	},	
...
{
	globals.log(cyan("Checking type validity for type: " + configItem.type));
...
var setMissingDefaults = function(configItem, property)
{
	...
	{	
		...
		for (var thisDefault in defaultsList)
		{
			...
			{
				// globals.log("Item type: %s, named: %s :Adding property %s with value: %s", cyan(configItem.type), cyan(configItem.name), cyan(defaultsList[thisDefault]), cyan(typeProperties[configItem.type].Properties[ key ]));
...
var hasMandatoryProperties = function(configItem)
{
	...
	if( (typeProperties[configItem.type] === undefined) || (typeProperties[configItem.type] == null) ) return false;
		...
		for (var index in mandatory)
		{
			...
			if (configItem[key] === undefined)
			{
				globals.log("Item named %s of type %s is missing its mandatory property: %s", cyan(configItem.name), cyan(configItem.type), cyan(key));
				return false;
			}
		}
		return true;
