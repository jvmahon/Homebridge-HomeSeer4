...
var hasValidProperties = function(configItem)
{
	...
	for (var property in configItem) {
		if (validPropertiesList.hasOwnProperty(property) == false)  { 
				var error = `config.json error: property: ${cyan(property)} is not a valid property for item named: ${cyan(configItem.name)} of type: ${cyan(configItem.type)}. \nValid Properties for this type are: ${cyan (Object.getOwnPropertyNames(typeProperties[configItem.type].Properties))}.`	
