...
class HomeSeerSystem 
{
	...
	/*
	sendDataValue = function(ref, newValue, sendOnlyOnChange = true) 
	{
		...
		if ( sendOnlyOnChange && (this.HomeSeerDevices[ref].status.value == newValue) )
			...
			// For some reason, that makes no sense at all, you have to print 'commandstring' to the console before sending it, or else HomeSeer indicates it got a bad command.
			console.log(yellow(`Sending new value ${newValue} to HomeSeer device: ${ref} using command string: ${commandstring}`));
			// console.log(yellow("Sending command string to HomeSeer: " + commandstring));
