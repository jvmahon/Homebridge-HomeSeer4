...
class HomeSeerSystem 
{
	...
	getValue(reference) {
		if(this.initialized === false) { throw new SyntaxError("HomeSeer System Object Is Not Initialized")};
	...
	sendDataValue(ref, newValue, sendOnlyOnChange = true)  {
		...
		if( ref === null || newValue === undefined ) throw new SyntaxError("Called updateValue with incorrect parameters. Need to specify both a reference and a Value. Reference was: " + ref + ", Value was: " + newValue);
	...
	registerObjectToReceiveUpdates(ref, object) {		
		...
		if (this.HomeSeerDevices[ref].notifyObjects.includes(object) === false) {
			...
			console.log("*Warning* - tried to add an item to HomeSeerDevices[ref].notifyObjects that already existed!");
		}
	}
	...
	getControlPairByUseType(reference, useValue) {
		if (this.initialized === false) { throw new SyntaxError("HomeSeer System Object Is Not Initialized")};
		if (isNaN(useValue)) throw new SyntaxError("Function getControlPairByUseType was passed a non-numberic UseValue")
			
		return this.HomeSeerDevices[reference]?.ControlPairs?.find(  (element) => { return ((element.Ref === reference) && ( element.ControlUse == useValue ))})
	}
	
	displayData(reference) {
		if(this.initialized === false) { throw new SyntaxError("HomeSeer System Object Is Not Initialized")};
		if (reference === undefined) {	
				console.log( JSON.stringify(this.HomeSeerDevices) );
		} else {
			console.log( JSON.stringify(this.HomeSeerDevices[reference]?.status) );
			console.log( JSON.stringify(this.HomeSeerDevices[reference]?.ControlPairs) );
		}
	}
