	...
	async initialize()
	{
		...
		var getControlInfo = promiseHTTP({ uri:controlURL.href, json:true, strictSSL:false})
					.then( function(HSControls)
					{
						...
						{
							that.HSDevices[currentDevice.ref] = [];
							that.HSDevices[currentDevice.ref] = {...currentDevice};
							// console.log(yellow("device is: " + that.HSDevices[currentDevice.ref]));
						}
		...
		let result = await Promise.all([getStatusInfo, getControlInfo]).then( function(response) 
